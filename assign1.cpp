...
// COMMENTS TO GRADER:
// In this part, I implemented reflection texture-mapping and generate an object shaped as torus. The image of my own texture is images/torus.jpg
// From my point of view, I got the same effects as example programs. In any case you can not run the program, please kindly contact me because I
// have put many effocts!
...
// Image for the torus object
const char torusTexFile[] = "images/torus.jpg";
...
void MakeReflectionImage( void )
{
    ...
    // STEP 6: Read the correct color buffer into the b   correct texture object.
    //********************************************************
	
	// STEP 1: Clear the color and depth buffer
	glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT );
	 
	// STEP 2: Set up the view volume for the imaginary viewpoint
    glMatrixMode( GL_PROJECTION );
    glLoadIdentity();
	glFrustum( TABLETOP_Y1 - eyePos[1], TABLETOP_Y2 - eyePos[1], TABLETOP_X1 - eyePos[0], TABLETOP_X2 - eyePos[0], eyePos[2] - TABLETOP_Z, 10.0 );
	
    glMatrixMode( GL_MODELVIEW );
    glLoadIdentity();

	// STEP 3: Set up the imaginary viewpoint.
	GLdouble eyePos_Z = 2 * TABLETOP_Z - eyePos[2];  
    gluLookAt( eyePos[0], eyePos[1], eyePos_Z, 
			   eyePos[0], eyePos[1], eyePos[2],
			   1.0, 0.0, 0.0 );

	// STEP 4: Set up the light source positions in the world space.
	glLightfv( GL_LIGHT0, GL_POSITION, light0Position );
    glLightfv( GL_LIGHT1, GL_POSITION, light1Position );

	// STEP 5: Draw all the scene except table
	DrawRoom();
	DrawTeapot();
	DrawSphere();
	DrawTorus();
...
// This texture object is for the torus texture map.

    glGenTextures( 1, &torusTexObj );
    glBindTexture( GL_TEXTURE_2D, torusTexObj );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );
    ...
    if ( ReadImageFile( torusTexFile, &imageData, 
                        &imageWidth, &imageHeight, &numComponents ) == 0 ) exit( 1 );
    if ( numComponents != 3 )
    {
        fprintf( stderr, "Error: Texture image is not in RGB format.\n" );
...
void DrawTorus( void )
{
    ...
    glPushMatrix();
    glTranslated( -0.5, 0.75, height + TABLETOP_Z );
...
// Tabletop.
	...
	// Set GL_MODULATE so that we can still see the diffuse color and lighting on the table-top
	glTexEnvf( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
	glBindTexture( GL_TEXTURE_2D, reflectionTexObj );
